...
@pytest.mark.parametrize("basename", ("config", "config-basic"))
def test_dump(tmpdir, datadir, basename, src_fmt, dst_fmt):
    with tmpdir.as_cwd():
        config = TimeVaryingConfig.from_file(datadir / f"{basename}.{src_fmt}")
